...
export const AdminPage: React.FC = () => {
  ...
  const limparForm = () => { setFotos([]); setTitulo(''); setTipo(''); setRegiao(''); setEndereco(''); setValor(''); setFinalidade('Venda'); setArea(70); setQuartos(2); setBanheiros(1); setVagas(1); setDescricao(''); setEditId(null); };
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      const payload = { titulo: titulo.trim(), tipo, regiao, endereco: endereco.trim(), valor: valorNum, finalidade, area: Number(area), quartos: Number(quartos), banheiros: Number(banheiros), vagas: Number(vagas), descricao: descricao.trim(), fotos: fotosParaEnviar };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-brand-bg">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-brand-muted text-sm tracking-wide">Carregando...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-brand-bg flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 flex items-center justify-center p-6 relative z-10">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[420px] bg-[#1d0a12]/80 backdrop-blur-xl rounded-[24px] shadow-[0_24px_64px_rgba(0,0,0,0.5)] p-8 md:p-10 border border-brand-light/10">
            ...
            <div className="text-center mb-8">
              ...
              <h1 className="font-serif text-[28px] font-light text-brand-light tracking-wide">Área da Silvia</h1>
              <p className="text-[13px] text-brand-muted mt-2 leading-relaxed font-light">Bem-vinda de volta, Silvia — digite sua senha</p>
              <p className="text-[11px] tracking-[0.18em] text-brand-gold/60 uppercase mt-1">CRECISP 125743 • Acesso restrito</p>
            </div>
            <form onSubmit={handleLogin} className="space-y-5">
              <div>
                <label className="block text-[11px] tracking-[0.18em] text-brand-muted uppercase font-light mb-2">Senha</label>
                <input type="password" value={senhaInput} onChange={(e) => setSenhaInput(e.target.value)} placeholder="Sua senha" className="w-full h-[52px] px-4 rounded-[12px] border border-brand-light/10 bg-brand-bg/60 text-[15px] text-brand-light placeholder:text-brand-muted/40 focus:outline-none focus:border-brand-gold focus:bg-brand-bg transition" autoFocus />
  ...
  if (modoForm) {
    return (
      <div className="min-h-screen bg-brand-bg pb-[96px]">
        ...
        <div className="sticky top-0 z-30 bg-brand-bg border-b border-brand-light/10">
          ...
          <div className="max-w-[720px] mx-auto px-4 h-[64px] flex items-center justify-between">
            <button onClick={() => { setModoForm(null); limparForm(); }} className="text-[13px] tracking-[0.14em] uppercase font-light text-brand-muted hover:text-brand-light flex items-center gap-2 px-2 py-2 -ml-2 transition">
              <ChevronLeft size={18} className="text-brand-gold" /> Voltar
            </button>
            <span className="font-serif text-[16px] text-brand-light tracking-wide flex items-center gap-2"><Sparkles size={14} className="text-brand-gold" /> {modoForm === 'novo' ? 'Novo imóvel' : 'Editar imóvel'}</span>
        ...
        <div className="max-w-[720px] mx-auto px-4 pt-6 space-y-5">
          ...
          <div className="bg-[#1d0a12]/60 backdrop-blur-xl rounded-[20px] p-5 md:p-6 shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-brand-light/10">
            ...
            <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Ex: Casa 3 quartos com quintal no Butantã" className="w-full h-[52px] px-4 rounded-[12px] border border-brand-light/10 bg-brand-bg/60 text-[15px] text-brand-light placeholder:text-brand-muted/40 focus:outline-none focus:border-brand-gold focus:bg-brand-bg transition" />
          </div>
          ...
          <div className="bg-[#1d0a12]/60 backdrop-blur-xl rounded-[20px] p-5 md:p-6 shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-brand-light/10">
            ...
            <input type="text" value={endereco} onChange={(e) => setEndereco(e.target.value)} placeholder="Rua, número e bairro" className="w-full h-[52px] px-4 rounded-[12px] border border-brand-light/10 bg-brand-bg/60 text-[14px] text-brand-light placeholder:text-brand-muted/40 focus:outline-none focus:border-brand-gold transition" />
          </div>
          ...
          <div className="bg-[#1d0a12]/60 backdrop-blur-xl rounded-[20px] p-5 md:p-6 shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-brand-light/10">
            ...
            <div className="relative">
              ...
              <input type="text" inputMode="numeric" value={valor} onChange={(e) => { const onlyNums = e.target.value.replace(/\D/g, ''); setValor(onlyNums); }} placeholder="520000" className="w-full h-[56px] pl-12 pr-4 rounded-[12px] border border-brand-light/10 bg-brand-bg/60 text-[18px] font-light text-brand-light placeholder:text-brand-muted/30 focus:outline-none focus:border-brand-gold transition" />
            </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-brand-bg border-t border-brand-light/10 p-4">
    ...
    <div className="min-h-screen bg-brand-bg">
      ...
      <div className="sticky top-0 z-30 bg-brand-bg border-b border-brand-light/10">
        ...
        <div className="max-w-[960px] mx-auto px-4 h-[64px] flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-brand-gold/10 border border-brand-gold/20 flex items-center justify-center text-brand-gold font-serif text-sm shadow-[0_0_12px_rgba(212,163,115,0.15)]">SH</div>
            <div>
              <h1 className="font-serif text-[18px] font-light text-brand-light tracking-wide">Meus Imóveis</h1>
              <p className="text-[11px] tracking-[0.14em] uppercase text-brand-muted font-light">Silvia Helena • {imoveis.length} imóveis • CRECISP 125743</p>
            </div>
          </div>
          <button onClick={handleSair} className="text-[12px] tracking-[0.12em] uppercase font-light text-brand-muted border border-brand-light/10 rounded-full px-4 py-2 hover:bg-white/[0.04] hover:border-brand-light/20 hover:text-brand-light transition flex items-center gap-1.5">
            <LogOut size={12} /> Sair
          </button>
        </div>
      </div>
